...
from line_depth_fixer import LineDepthFixer
...
class ExportWorker(QObject):
    ...
    def run(self):
        try:
            ...
            # 10%: Fixer 생성
            line_depth_fixer = LineDepthFixer(
            ...
            line_depth_fixer.shape_union_run()
            self.progress.emit(70)

            # 80%: 출력 파일 구성
            self.status.emit("출력 파일 구성 중...")
            output_file = line_depth_fixer.configure_output_file("gpkg")
            self.progress.emit(80)

            # 80% -> 100%: GPKG 내보내기
            self.status.emit("GeoDataFrame을 GeoPackage 내보내기 중...")
            line_depth_fixer.export_lines_to_geopackage(output_file, self.layer_name)
